...
function FigmaEmbed(props) {
  ...
  return (
    <div className="figma-embed">
      {image &&
        <img src={image} alt={props.frame}/>
      }
      { !image && !errorMessage &&
        <div className="figma-embed__loader">{loadingMessage ? loadingMessage : `Loading “${props.frame}” from Figma…`}</div>
      }
      { errorMessage &&
        <div className="figma-embed__error">{errorMessage}</div>
      }

      <a className="figma-embed__via" href={`https://www.figma.com/file/${props.document}${frameId ? `?node-id=${frameId}` : ""}`} target="_blank" rel="noopener noreferrer">Edit in Figma</a>
    </div>
  );
}
