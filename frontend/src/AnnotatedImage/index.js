    ...
    <div className="annotated-image">
      ...
      <div className="annotated-image__stage">
        ...
        {p.annotations.annnotations.map((a, i) => {
          ...
          return <div
          className={`annotated-image__marker ${highlightedAnnotation === i ? "annotated-image__marker--highlight" : ""}`}
          style={{left: x, top: y}}
            onMouseEnter={() => { setHighlightedAnnotation(i)}}
            onMouseLeave={() => { setHighlightedAnnotation(null)}}
            key={i}
          >
            <div className="annotated-image__badge annotated-image__badge--highlight" style={{backgroundColor: p.annotations.annotationColor}}>{i + 1}</div>
          </div>
        })}
      </div>
      </ComponentDemo>
      ...
      <div className="annotated-image__annotations">
        {p.annotations.annnotations.map((a, i) => {
          ...
          >
            <div
              className={`annotated-image__badge ${highlightedAnnotation === i ? "annotated-image__badge--highlight" : ""}`}
