...
function TabBar(props) {
  ...
  tabs = props.tabs && props.tabs.map((t, i) => {
    ...
    return <a href={`#${t}`} className={classes.join(" ")} key={t} onClick={(ev) => { ev.preventDefault(); props.onSetActiveTab(t); }}>{t}</a>;
  });

  let rightSideTabs = [];

  rightSideTabs = props.rightSideTabs && props.rightSideTabs.map((t, i) => {
    let classes = ["tab-bar__tab"];
