...
function ColorCard(props) {
  ...
  return (<div className={classes.join(" ")} key={props.id} onClick={copyCode}>
    <div className="color-card__demo" style={{ backgroundColor: props.color }}>
      ...
      <div
        className={`color-card__copied-indicator ${showCopiedIndicator ? "color-card__copied-indicator--is-visible" : ""}`}
        style={{ color: contrast.ratio(props.color, "white") < contrast.ratio(props.color, "black") ? "black" : "white"}}
      >
        Copied!
      </div>
    </div>
    <div className="color-card__name">{props.children} <span className="color-card__id">({props.id})</span></div>
